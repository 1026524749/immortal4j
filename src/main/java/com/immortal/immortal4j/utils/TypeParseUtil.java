package com.immortal.immortal4j.utils;

import com.immortal.immortal4j.exception.BizException;
import com.immortal.immortal4j.support.DataType;
...
public final class TypeParseUtil {
	...
	public static final Object convert(Object value, String type, String format) {
		...
		if (value == null) {
			...
			throwExcption(value.getClass().getName(),type);
		}
		return  null;
	}
	...
	public static void throwExcption(String from,String to) throws BizException{
		switch (to){
			case "String": throw new BizException(HttpCode.TYPE_PARSE_STRING);
			case "Boolean": throw new BizException(HttpCode.TYPE_PARSE_BOOLEAN);
			case "Integer": throw new BizException(HttpCode.TYPE_PARSE_INTEGER);
			case "Double": throw new BizException(HttpCode.TYPE_PARSE_DOUBLE);
			case "Float": throw new BizException(HttpCode.TYPE_PARSE_FLOAT);
			case "BigDecimal": throw new BizException(HttpCode.TYPE_PARSE_BIGDECIMAL);
			case "Date": throw new BizException(HttpCode.TYPE_PARSE_DATE);
			case "Time": throw new BizException(HttpCode.TYPE_PARSE_TIME);
			case "Timestamp": throw new BizException(HttpCode.TYPE_PARSE_TIMESTAMP);
			default:throw new BizException(HttpCode.TYPE_PARSE_OTHER);
		}
	}
}
